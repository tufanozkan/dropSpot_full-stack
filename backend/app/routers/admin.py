...
def create_new_drop(drop: schemas.DropCreate, db: Session = Depends(database.get_db)):
    return crud.create_drop(db=db, drop=drop)

@router.get("/drops", response_model=List[schemas.Drop])
def read_all_drops(skip: int = 0, limit: int = 100, db: Session = Depends(database.get_db)):
    return crud.get_drops(db, skip=skip, limit=limit)

@router.put("/drops/{drop_id}", response_model=schemas.Drop)
def update_existing_drop(drop_id: int, drop_update: schemas.DropUpdate, db: Session = Depends(database.get_db)):
...
def delete_existing_drop(drop_id: int, db: Session = Depends(database.get_db)):
